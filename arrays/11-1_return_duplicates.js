...
function orderedIntersection(sortedA, sortedB) {
    ...
    let idxA = 0;
    let idxB = 0;
    while(sortedA[idxA] != undefined && sortedB[idxB] != undefined) {
        if(sortedA[idxA] < sortedB[idxB]) {
            ...
            if (newArr[newArr.length-1] !== sortedA[idxA]) {
                newArr.push(sortedA[idxA]);
            }
            idxA++;
            idxB++;
        }
...
console.log(orderedIntersection([2,4,4,4], [1,2,4,4,4,4,4,5,8,8]));
