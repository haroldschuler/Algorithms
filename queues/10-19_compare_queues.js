...
class Queue {
   ...
   */
    compareQueues(q2) {
        ...
        let tempQueue1 = new Queue();
        let tempQueue2 = new Queue();
        while(this.length > 0) {
            if(this.head.data != q2.head.data) {
                return false;
            }
            tempQueue1.enqueue(this.head.data);
            tempQueue2.enqueue(q2.head.data);
            this.dequeue();
            q2.dequeue();
        }
        while(tempQueue1.length > 0){
            this.enqueue(tempQueue1.head.data);
            q2.enqueue(tempQueue2.head.data);
            tempQueue1.dequeue();
            tempQueue2.dequeue();
        }
        return true;
    ...
    // doesn't work - need to change enqueue function to take in data and not the node
    isPalindrome() {
        let tempQ = new Queue();
        let tempStack = new Stack();
        let isPal = true;
